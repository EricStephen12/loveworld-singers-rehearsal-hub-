...
interface PracticePageProps {
  onBack: () => void;
  onKaraokeStateChange: (isActive: boolean) => void;
}

const PracticePage = ({ onBack, onKaraokeStateChange }: PracticePageProps) => {
  ...
  const handlePracticeStart = (mode: string) => {
  ...
  if (showKaraokeMode) {
    ...
    return (
      <div className="karaoke-mode-placeholder">
        <h2>Karaoke Mode</h2>
        <p>Karaoke functionality coming soon!</p>
        <button onClick={handleBackFromKaraoke} className="back-button">
          Back to Practice
        </button>
      </div>
    );
